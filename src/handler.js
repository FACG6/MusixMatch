...
const handleSearchById = (request, response) => {
  if (request.url === '/search') {
    ...
    request.on('end', () => {
      ...
      const q_track = allData.split(' ').join('%20');
      const track_method = 'track.lyrics.get';
      const options = {
...
const handleKeywords = (request, response) => {
  ...
  request.on('end', () => {
    const API_KEY = process.env.apiKey;
    const dataArray = JSON.parse(allData);
    const f_has_lyrics = dataArray[0];
    const language = dataArray[1];
    const track_method = 'track.search';
    const options = {
      url: `https://api.musixmatch.com/ws/1.1/${track_method}?format=json&callback=callback&q_lyrics=${f_has_lyrics}&f_lyrics_language=${language}&quorum_factor=1&apikey=${API_KEY}`,
      method: 'GET',
      headers: {
        Accept: 'application/json',
        'Accept-Charset': 'utf-8',
    ...
    req(options, (err, res, body) => {
      if (err) {
        ...
        response.end('<h1>Server Error</h1>');
      } else if (JSON.parse(body).message.body.track_list){
          console.log(JSON.parse(body).message.body.track_list);
                        response.writeHead(200, {
                            'Content-Type': 'application/json',
                        });
...
const handleError = (request, response) => {
  ...
  module.exports = {
 handleHomePage, handleSearchById, handleKeywords, handleStatics, handleError 
};
